...
import { CycleTransactionType } from './data/cycleStorage';
...
export default function App() {
  ...
  return <div className="min-h-screen bg-stone-100 text-stone-800 flex flex-col items-center">
    ...
    <main className={`w-full flex-1 ${isPhoneFrame ? 'max-w-md my-0 sm:my-4 sm:rounded-[36px] bg-white sm:border sm:border-stone-200 sm:shadow-xl sm:overflow-hidden min-h-[calc(100vh-60px)] sm:min-h-[760px] flex flex-col' : 'max-w-3xl my-0 sm:my-6 px-4'}`}>
      {isPhoneFrame && <div className="hidden sm:flex items-center justify-between px-6 pt-3 pb-1 text-[11px] font-semibold text-stone-400"><span>2026.09.15</span><div className="w-20 h-3.5 bg-stone-900 rounded-full mx-auto" /><span>100% 🔋</span></div>}
      <div className="flex-1 p-4 sm:p-5 overflow-y-auto">{currentTab === 'home' && <HomeScreen onOpenAddModal={handleOpenAddModal} onEditRecord={handleEditRecord} onNavigateTab={setCurrentTab} />}{currentTab === 'history' && <HistoryScreen onEditRecord={handleEditRecord} onOpenAddModal={() => handleOpenAddModal('saving')} />}</div>
    </main>
    <Navigation currentTab={currentTab} onSelectTab={setCurrentTab} onQuickAdd={() => handleOpenAddModal('saving')} />
    <SavingRecordModal isOpen={modalOpen} onClose={() => { setModalOpen(false); setRecordToEdit(null); }} recordToEdit={recordToEdit} onSaved={() => {}} cycleTransactionType={modalType} />
