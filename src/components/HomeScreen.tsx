...
import { ArrowDownRight, ArrowUpRight, Calendar, ChevronRight, CircleDollarSign, Gift, PiggyBank, WalletCards } from 'lucide-react';
...
interface HomeScreenProps { onOpenAddModal: (type?: CycleTransactionType) => void; onEditRecord: (record: SavingRecord) => void; onNavigateTab: (tab: 'home' | 'record' | 'history') => void; }
const money = (value: number) => formatCurrencyKRW(Math.max(0, Math.round(value)));
const FALLBACK_APP: AppTechItem = { id: 'saving-app', name: '오늘의 절약 앱 추천', description: '걷기·출석·미션으로 포인트를 모아 생활비 절약에 활용해 보세요.', category: '절약', referralCode: '', referralUrl: '', isActive: true };
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onOpenAddModal, onEditRecord, onNavigateTab }) => {
  ...
  useEffect(() => { if (todayKey !== cycle.startDate) return; const previousCycle = getCycleForDate(getPreviousDate(today), payday); if (CycleStorage.hasSettled(previousCycle.startDate)) return; const amount = getCycleSettlementAmount(previousCycle, records, transactions); if (amount > 0) StorageRepository.saveRecord({ amount, date: todayKey, memo: '급여일 직전 잔액 자동 저축' }); CycleStorage.markSettled(previousCycle.startDate); }, [cycle.startDate, payday, records, today, todayKey, transactions]);
  ...
  const transactionRows = [...cycleTransactions.map((t) => ({ id: t.id, date: t.date, memo: t.memo, amount: t.amount, type: t.type })), ...cycleRecords.map((r) => ({ id: r.id, date: r.date, memo: r.memo, amount: r.amount, type: 'saving' as const }))].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 4);
  ...
  return <div className="space-y-4 pb-20">
    <div className="flex items-center justify-between pt-1 pb-1"><div className="flex items-center gap-2"><Calendar className="w-5 h-5 text-stone-500" /><h1 className="text-xl sm:text-2xl font-black text-stone-900 tracking-tight">{monthLabel}</h1></div><button type="button" onClick={() => onOpenAddModal('salary')} className="inline-flex items-center gap-1.5 bg-stone-100 hover:bg-emerald-50 text-stone-700 hover:text-emerald-800 border border-stone-200 hover:border-emerald-200 px-3 py-1.5 rounded-full text-xs font-bold transition-colors"><WalletCards className="w-3.5 h-3.5" />월급일 {payday}일</button></div>
    ...
    <div className="p-5 sm:p-6 bg-white rounded-3xl border border-stone-200 shadow-sm">
      ...
      <div className="relative"><div className="w-full h-8 rounded-full bg-stone-100 border border-stone-200 overflow-hidden flex" title="막대의 각 영역에 마우스를 올려 파이 속성과 퍼센트를 확인하세요.">{segment('saving', savedPct, 'bg-emerald-500')}{segment('spending', spentPct, 'bg-stone-400')}{segment('remaining', remainingPct, 'bg-stone-100')}</div>{hoveredPie && <div className="absolute left-1/2 -translate-x-1/2 top-10 z-10 px-3 py-2 rounded-xl bg-stone-900 text-white shadow-lg pointer-events-none whitespace-nowrap text-[11px] font-bold">{PIE_LABEL[hoveredPie]} · {Math.round(pieDetails[hoveredPie].pct)}% · {money(pieDetails[hoveredPie].amount)}</div>}</div>
    ...
    <div className="p-4 sm:p-5 bg-gradient-to-r from-amber-50/70 via-stone-50 to-emerald-50/40 rounded-3xl border border-stone-200/80 shadow-xs flex items-center justify-between gap-3"><div className="flex items-center gap-3 min-w-0"><div className="w-14 h-12 rounded-2xl bg-amber-400/20 text-amber-900 flex items-center justify-center font-black text-xs border border-amber-300 shrink-0">{rankInfo.title}</div><div className="min-w-0"><h2 className="text-sm font-bold text-stone-800">{rankInfo.title}</h2><div className="text-xs text-stone-500 mt-0.5">연간 {rankInfo.currentLevelXp.toLocaleString()} / {rankInfo.xpForNextLevel.toLocaleString()} XP</div><div className="w-32 sm:w-44 bg-stone-200/80 h-1.5 rounded-full mt-1.5 overflow-hidden"><div className="bg-amber-500 h-full rounded-full" style={{ width: `${Math.round(rankInfo.levelProgressRatio * 100)}%` }} /></div></div></div><span className="text-[11px] font-semibold text-emerald-700 bg-emerald-100/70 px-2 py-1 rounded-lg shrink-0">{currentYear}년 {money(annualSavings)}</span></div>
