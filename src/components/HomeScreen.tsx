...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onOpenAddModal, onEditRecord, onEditTransaction, onNavigateTab }) => {
  ...
  const salary = monthTransactions.filter((t) => t.type === 'salary').reduce((s, t) => s + Math.max(0, t.amount), 0);
  const extraIncome = monthTransactions.filter((t) => t.type === 'income').reduce((s, t) => s + Math.max(0, t.amount), 0);
  ...
  return <div className="space-y-4 pb-20">
    ...
    <div className="p-5 sm:p-6 bg-white rounded-3xl border border-stone-200 shadow-sm">
      ...
      <div className="grid grid-cols-4 gap-2 mt-4"><button onClick={() => onOpenAddModal('saving')} className="py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center justify-center gap-1"><PiggyBank className="w-4 h-4" />저축</button><button onClick={() => onOpenAddModal('salary')} className="py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold flex items-center justify-center gap-1"><WalletCards className="w-4 h-4" />급여</button><button onClick={() => onOpenAddModal('expense')} className="py-2.5 rounded-xl bg-stone-800 hover:bg-stone-900 text-white text-xs font-bold flex items-center justify-center gap-1"><ArrowDownRight className="w-4 h-4" />소비</button><button onClick={() => onOpenAddModal('income')} className="py-2.5 rounded-xl bg-white hover:bg-emerald-50 text-stone-700 border border-stone-200 hover:border-emerald-200 text-xs font-bold flex items-center justify-center gap-1"><ArrowUpRight className="w-4 h-4 text-emerald-600" />수입</button></div>
      {!hasSalary && <div className="mt-2.5 text-center py-2 text-[11px] text-stone-400">이번 달 급여가 아직 기록되지 않았습니다.</div>}
    </div>
