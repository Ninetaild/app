...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onOpenAddModal, onEditRecord, onNavigateTab }) => {
  ...
  useEffect(() => {
    ...
    }).catch(() => { if (mounted) setRandomAppTech(FALLBACK_APP); });
  ...
  const pieDetails: Record<PieKind, { pct: number; amount: number }> = {
    saving: { pct: savedPct, amount: saved }, spending: { pct: spentPct, amount: spending }, remaining: { pct: remainingPct, amount: available },
  };
  ...
  return (
    <div className="space-y-4 pb-20">
      <div className="flex items-center justify-between pt-1 pb-1">
        <div className="flex items-center gap-2"><Calendar className="w-5 h-5 text-stone-500" /><h1 className="text-xl sm:text-2xl font-black text-stone-900 tracking-tight">{monthLabel}</h1></div>
      ...
      <div className="p-5 sm:p-6 bg-white rounded-3xl border border-stone-200 shadow-sm">
        ...
        <div className="relative">
          <div className="w-full h-8 rounded-full bg-stone-100 border border-stone-200 overflow-hidden flex" title="막대의 각 영역에 마우스를 올려 파이 속성과 퍼센트를 확인하세요.">{segment('saving', savedPct, 'bg-emerald-500')}{segment('spending', spentPct, 'bg-stone-400')}{segment('remaining', remainingPct, 'bg-stone-100')}</div>
          {hoveredPie && <div className="absolute left-1/2 -translate-x-1/2 top-10 z-10 px-3 py-2 rounded-xl bg-stone-900 text-white shadow-lg pointer-events-none whitespace-nowrap text-[11px] font-bold">{PIE_LABEL[hoveredPie]} · {Math.round(pieDetails[hoveredPie].pct)}% · {money(pieDetails[hoveredPie].amount)}</div>}
        </div>
      ...
      <div className="p-4 sm:p-5 bg-gradient-to-r from-amber-50/70 via-stone-50 to-emerald-50/40 rounded-3xl border border-stone-200/80 shadow-xs flex items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-2xl bg-amber-400/20 text-amber-900 flex items-center justify-center font-black text-sm border border-amber-300 shrink-0">{rankInfo.title}</div><div><h2 className="text-sm font-bold text-stone-800">{rankInfo.title}</h2><div className="text-xs text-stone-500 mt-0.5">연간 {rankInfo.currentLevelXp.toLocaleString()} / {rankInfo.xpForNextLevel.toLocaleString()} XP</div><div className="w-32 sm:w-44 bg-stone-200/80 h-1.5 rounded-full mt-1.5 overflow-hidden"><div className="bg-amber-500 h-full rounded-full" style={{ width: `${Math.round(rankInfo.levelProgressRatio * 100)}%` }} /></div></div></div><span className="text-[11px] font-semibold text-emerald-700 bg-emerald-100/70 px-2 py-1 rounded-lg shrink-0">{currentYear}년 {money(annualSavings)}</span></div>
