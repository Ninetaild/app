...
import { CycleStorage, CycleTransaction, getCycleForDate, getPreviousDate } from '../data/cycleStorage';
...
export const HistoryScreen: React.FC = () => {
  ...
  const loadData = () => {
    ...
    const previousCycle = getCycleForDate(getPreviousDate(new Date(`${currentCycle.startDate}T12:00:00`)), payday);
  ...
  return <div className="space-y-4 pb-20">
    ...
    </> : <div className="bg-white rounded-3xl border border-stone-200 p-5 shadow-xs"><h2 className="text-sm font-bold text-stone-800 mb-3">{selectedYear}년</h2><div className="divide-y divide-stone-100">{Array.from({ length: 12 }, (_, index) => index).map((monthIndex) => { const cycle = getCycleStartForMonth(selectedYear, monthIndex, payday); const tx = allTransactions.filter((t) => t.date >= cycle.startDate && t.date < cycle.endDate); const i = tx.filter((t) => t.type === 'income').reduce((s, t) => s + Math.max(0, t.amount), 0); const e = tx.filter((t) => t.type === 'expense').reduce((s, t) => s + Math.abs(t.amount), 0); return <div key={cycle.startDate} className="py-2.5 flex items-center justify-between"><div><span className="text-xs font-bold text-stone-700">{monthIndex + 1}월</span></div><div className="flex gap-3 text-[11px]"><span className="text-stone-500">수입 {money(i)}</span><span className="text-stone-500">소비 {money(e)}</span><span className="font-black text-emerald-700">저축 {money(Math.max(0, i - e))}</span></div></div>; })}</div></div>}
  </div>;
};
