...
export function saveSheetsUrl(url: string): void {
  const match = url.trim().match(/\/spreadsheets\/d\/([a-zA-Z0-9_-]+)/);
  if (match?.[1]) saveSheetsConnection(match[1], url.trim());
  else if (!url.trim()) clearSheetsConnection();
}
...
export function clearSheetsConnection(): void {
  setStored(STORAGE_KEYS.SPREADSHEET_ID, ''); setStored(STORAGE_KEYS.SPREADSHEET_URL, '');
  setStored(STORAGE_KEYS.LAST_SYNC_TIME, ''); setStored(STORAGE_KEYS.LAST_SYNC_STATUS, '');
}
...
async function createSpreadsheet(): Promise<SpreadsheetInfo> {
  const created = await googleApi<SpreadsheetInfo>('/spreadsheets', {
    method: 'POST',
    body: JSON.stringify({
      properties: { title: `저축 게임 백업 ${new Date().toLocaleDateString('ko-KR')}` },
...
async function writeRange(spreadsheetId: string, range: string, values: unknown[][]): Promise<void> {
  await googleApi(`/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}?valueInputOption=USER_ENTERED`, { method: 'PUT', body: JSON.stringify({ range, majorDimension: 'ROWS', values }) });
}
async function clearRange(spreadsheetId: string, range: string): Promise<void> {
  await googleApi(`/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}:clear`, { method: 'POST', body: JSON.stringify({}) });
}
async function formatSheets(spreadsheetId: string, spreadsheet: SpreadsheetInfo): Promise<void> {
  const sheetIds = new Map((spreadsheet.sheets || []).map((sheet) => [sheet.properties?.title, sheet.properties?.sheetId] as const).filter((entry): entry is readonly [string, number] => typeof entry[0] === 'string' && typeof entry[1] === 'number'));
  const requests: unknown[] = [];
  REQUIRED_SHEETS.forEach((title) => {
    const sheetId = sheetIds.get(title); if (sheetId === undefined) return;
    requests.push(
      { repeatCell: { range: { sheetId, startRowIndex: 0, endRowIndex: 1 }, cell: { userEnteredFormat: { textFormat: { bold: true }, horizontalAlignment: 'CENTER' } }, fields: 'userEnteredFormat.textFormat.bold,userEnteredFormat.horizontalAlignment' } },
      { updateSheetProperties: { properties: { sheetId, gridProperties: { frozenRowCount: 1 } }, fields: 'gridProperties.frozenRowCount' } },
      { autoResizeDimensions: { dimensions: { sheetId, dimension: 'COLUMNS', startIndex: 0, endIndex: title === '급여일' || title === '목표 저축액' ? 2 : 5 } } },
    );
  });
  const currency = (title: typeof REQUIRED_SHEETS[number], columnIndex: number) => {
    const sheetId = sheetIds.get(title); if (sheetId === undefined) return;
    requests.push({ repeatCell: { range: { sheetId, startRowIndex: 1, startColumnIndex: columnIndex, endColumnIndex: columnIndex + 1 }, cell: { userEnteredFormat: { numberFormat: { type: 'NUMBER', pattern: '#,##0"원"' } } }, fields: 'userEnteredFormat.numberFormat' } });
  };
  currency('수입', 2); currency('저축', 2); currency('소비', 2); currency('목표 저축액', 1);
  if (requests.length) await googleApi(`/spreadsheets/${encodeURIComponent(spreadsheetId)}:batchUpdate`, { method: 'POST', body: JSON.stringify({ requests }) });
}
...
export async function backupToGoogleSheets(records: SavingRecord[], goals: Record<string, number>, settings: AppSettings): Promise<boolean> {
  ...
  try {
    ...
    if (existingId) {
      try { spreadsheet = await googleApi<SpreadsheetInfo>(`/spreadsheets/${encodeURIComponent(existingId)}?includeGridData=false`); await ensureRequiredSheets(spreadsheet); }
      catch { clearSheetsConnection(); spreadsheet = await createSpreadsheet(); }
    } else spreadsheet = await createSpreadsheet();
    ...
    await Promise.all([
      writeRange(id, '급여일!A1', rows.payday), writeRange(id, '수입!A1', rows.income), writeRange(id, '저축!A1', rows.saving),
      writeRange(id, '소비!A1', rows.expense), writeRange(id, '목표 저축액!A1', rows.goal),
    ]);
    ...
    saveSheetsConnection(id, latest.spreadsheetUrl || spreadsheet.spreadsheetUrl || spreadsheetUrl(id));
...
// Data changes are intentionally local-only. Cloud writes happen only from the explicit backup button.
...
export async function getSpreadsheetMetadata(spreadsheetId: string): Promise<SpreadsheetInfo> {
  return googleApi<SpreadsheetInfo>(`/spreadsheets/${encodeURIComponent(spreadsheetId)}?includeGridData=false`);
}
